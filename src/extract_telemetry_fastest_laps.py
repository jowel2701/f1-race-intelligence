...
def save_incremental_csv(dataframe: pd.DataFrame, output_path: str) -> None:
    if dataframe.empty:
        return
...
def main() -> None:
    ...
    if not os.path.exists(laps_path):
        raise FileNotFoundError( "No existe data/raw/laps_raw.csv. "
    ...
    for _, fastest_lap in tqdm(
        ...
        print(f"\nSession {session_key} | " f"Driver {driver_number} | " f"Lap {lap_number}")
        ...
        car_data_fastest_lap = windows_fast_lap(
            car_data,
            fastest_lap,
        )

        location_fastest_lap = windows_fast_lap(
            location,
            fastest_lap,
        )
        ...
        save_incremental_csv(
            car_data_fastest_lap,
            CAR_DATA,
        )

        save_incremental_csv(
            location_fastest_lap,
            LOCATION_DATA,
        )
